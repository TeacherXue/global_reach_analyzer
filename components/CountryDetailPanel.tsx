...
const CountryDetailPanel: React.FC<CountryDetailPanelProps> = ({ iso3, onClose }) => {
    ...
    return (
        <>
            ...
            {/* Panel */}
            <div
                className={`fixed right-0 top-0 h-full w-[440px] max-w-[90vw] bg-slate-900 border-l border-slate-700/70 shadow-2xl shadow-black/40 z-50 flex flex-col transition-transform duration-300 ease-out ${isVisible ? 'translate-x-0' : 'translate-x-full'}`}
            >
                {/* Header */}
                <div className="flex-shrink-0 border-b border-slate-700/70 bg-slate-800/60">
                        ...
                        {TABS.map(tab => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {tab.label}
                                    {badge !== null && (
                                        <span className={`text-[10px] px-1.5 py-0.5 rounded-full ml-1 ${isActive ? 'bg-blue-500/20 text-blue-400' : 'bg-slate-600 text-slate-400'
                                            }`}>
                                            {badge}
                                        </span>
                                    )}
                ...
                <div className="flex-1 overflow-y-auto">
                    {activeTab === 'overview' && countryInfo && (
                        <OverviewTab countryInfo={countryInfo} timeInfo={timeInfo} />
                    )}
...
const OverviewTab: React.FC<OverviewTabProps> = ({ countryInfo, timeInfo }) => {
        ...
        <div className="p-4 space-y-4">
            ...
            <div className="bg-slate-800/70 rounded-xl p-4 border border-slate-700/50">
                ...
                <div className="space-y-2.5 text-sm">
                    <InfoRow label="母语" value={countryInfo.primaryLanguage} />
                    {countryInfo.secondaryLanguages.length > 0 && (
                        <InfoRow label="第二语言" value={countryInfo.secondaryLanguages.join(', ')} />
                    )}
                    <InfoRow
                        label="英语水平"
            ...
            <div className="bg-slate-800/70 rounded-xl p-4 border border-slate-700/50">
                ...
                <div className="space-y-2.5 text-sm">
                    <InfoRow label="开发信语言" value={countryInfo.businessLang} valueClass="text-green-400 font-bold" />
                    {countryInfo.businessLangAlt && (
                        <InfoRow label="备选语言" value={countryInfo.businessLangAlt} />
                    )}
                    <InfoRow label="Google地图" value={countryInfo.googleMapsLang} />
                    ...
                    <div className="flex justify-between gap-4">
                        <span className="text-slate-400">WhatsApp 普及度</span>
                ...
                {countryInfo.businessNote && (
                    <div className="mt-3 p-2.5 bg-slate-700/40 rounded-lg text-xs text-slate-300 italic leading-relaxed">
                        💡 {countryInfo.businessNote}
                    </div>
                )}
            </div>
            ...
            {timeInfo && (
                <div className="bg-slate-800/70 rounded-xl p-4 border border-slate-700/50">
                    ...
                    <div className="space-y-2.5 text-sm">
                        ...
                        <InfoRow label="时区" value={timeInfo.utcOffset} />
                    </div>
                    <div className="mt-3 p-2.5 bg-amber-900/20 rounded-lg border border-amber-800/30">
                        <div className="text-xs text-amber-300 mb-1.5 font-medium">🇨🇳 最佳联系时间（北京时间）</div>
                        <div className="flex justify-between text-xs">
                            <span className="text-slate-300 flex items-center gap-1">
                                <Sun className="w-3 h-3 text-amber-400" />
                                早间: <span className="text-white font-bold ml-1">{timeInfo.morningBj}</span>
                            </span>
                            <span className="text-slate-300 flex items-center gap-1">
                                <Moon className="w-3 h-3 text-blue-400" />
                                午后: <span className="text-white font-bold ml-1">{timeInfo.afternoonBj}</span>
                            </span>
                        </div>
                    </div>
                </div>
...
const YellowPagesTab: React.FC<YellowPagesTabProps> = ({ iso3, yellowPages, categories, countryName }) => {
    if (!yellowPages || categories.length === 0) {
        return (
            <div className="p-8 text-center">
                <div className="text-4xl mb-3">📋</div>
                <p className="text-slate-400 text-sm">暂无 {countryName} 的黄页数据</p>
                <p className="text-slate-500 text-xs mt-1">Yellow Pages data not available yet</p>
            </div>
        );
    }
    ...
    return (
        <div className="p-4 space-y-3">
            {categories.map(category => {
                ...
                return (
                    <div key={category.id} className="bg-slate-800/50 rounded-xl border border-slate-700/40 overflow-hidden">
                        {/* Category Header */}
                        <div className="px-4 py-2.5 bg-slate-800/80 border-b border-slate-700/40 flex items-center gap-2">
                            <span className="text-base">{category.icon}</span>
                            <span className="text-sm font-semibold text-white">{category.name}</span>
                            <span className="text-xs text-slate-500 ml-auto">{entries.length}</span>
                        </div>
                        {/* Entries */}
                        <div className="divide-y divide-slate-700/30">
                            {entries.map((entry, idx) => (
                                <a
                                    ...
                                    className="group flex items-start gap-3 px-4 py-3 hover:bg-slate-700/30 transition-colors cursor-pointer"
                                >
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 mb-0.5">
                                            <span className="text-sm font-medium text-blue-400 group-hover:text-blue-300 transition-colors truncate">
                                                {entry.name}
                                            </span>
                                            <ExternalLink className="w-3 h-3 text-slate-500 group-hover:text-blue-400 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" />
                                        </div>
                                        <p className="text-xs text-slate-400 leading-relaxed line-clamp-2">{entry.description}</p>
